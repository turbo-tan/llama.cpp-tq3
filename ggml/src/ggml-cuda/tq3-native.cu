...
static __global__ void tq3_rotate_act_copy_kernel(float * __restrict__ dst, const float * __restrict__ src, int64_t n) {
    const int64_t base = (int64_t)blockIdx.x * QK_TQ3_0;
    if (base >= n) return;
    const int lane = threadIdx.x;
...
void ggml_cuda_tq3_rotate_act_copy(float * dst, const float * src, int64_t n, cudaStream_t stream) {
    tq3_rotate_act_copy_kernel<<<n / QK_TQ3_0, QK_TQ3_0, 0, stream>>>(dst, src, n);
}
