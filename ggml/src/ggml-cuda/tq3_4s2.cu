...
static __global__ void ggml_cuda_tq3_4s2_dot_kernel(
...
static bool ggml_cuda_tq3_4s2_enabled_runtime() {
...
void ggml_cuda_tq3_4s2_launch_dot(
    ...
    if (!ggml_cuda_tq3_4s2_enabled_runtime()) {
        return;
    }

    ggml_cuda_tq3_4s2_dot_kernel<<<nblocks, WARP_SIZE, 0, stream>>>(in, act, out, nblocks);
}
