    ...
    // in-graph argmax and hidden state, so one decode drafts n_tokens tokens
    {
        ...
        if (dbg) {
            fprintf(stderr, "CHAINGRAPH mtp_chain=%d n_tokens=%d n_seqs=%d token=%d\n",
                (int) (std::getenv("LLAMA_SPEC_CHAIN") != nullptr), (int) n_tokens, (int) ubatch.n_seqs, ubatch.token != nullptr);
            fflush(stderr);
        }
    }
    if ((std::getenv("LLAMA_SPEC_CHAIN") != nullptr) && n_tokens > 1 && n_tokens <= 8 && ubatch.n_seqs_unq == 1 && ubatch.token && ubatch.output && ubatch.output[0]) {
